...
- (effort: M) Add the remaining adversarial decrypt-path tests (C4 tail) — `igloo-home` + `igloo-pwa` · shared NIP-44 wrapper adversarial coverage is in place; remaining tails are TS-side unlock/rotate/recover handler tests (home) and a real-WASM (un-mocked) wrong-password/corrupted-package test (pwa).
