...
- (effort: M) Upgrade `igloo-pwa` to Vite 8 to clear Vite/esbuild dev-server advisories — `igloo-pwa` · a breaking major bump; current `npm audit --omit=dev` is clean, while full audit reports only dev-server exposure (esbuild CORS/readback, Vite optimized-deps path handling, Windows launch-editor/`server.fs.deny` paths) that is not in the shipped static bundle, so it is deferred rather than rushed.
