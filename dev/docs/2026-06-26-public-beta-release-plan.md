...
| **C4** adversarial decrypt tests | 🟡 PARTIAL | Test-depth tail only: `igloo-shared` covers app-facing NIP-44 validation/ECDH/malformed-ciphertext paths, and `igloo-home` covers unlock/rotate/recover TS-side handler failures. Remaining tail is the real-WASM adversarial path (`igloo-pwa`). Error handling already exists. Fast-follow candidate, not a hard blocker. |
