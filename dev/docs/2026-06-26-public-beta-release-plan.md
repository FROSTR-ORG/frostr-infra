...
| **C4** adversarial decrypt tests | 🟡 PARTIAL | Test-depth tail only: `igloo-shared` now covers app-facing NIP-44 validation, ECDH failure propagation, pending-state cleanup, and malformed ciphertext rejection in `wasm-bridge-node.test.ts`; remaining tails are TS-side handler tests (`igloo-home`) and a real-WASM adversarial path (`igloo-pwa`). Error handling already exists. Fast-follow candidate, not a hard blocker. |
