...
test.describe('provider bridge live signer', () => {
  ...
  test('invite challenges are single-use for manual onboard requests', async ({
    liveSigner
  }) => {
    const firstCount = await liveSigner.requestOnboardNonceCount();

    expect(firstCount).toBeGreaterThan(0);
    await expect(liveSigner.requestOnboardNonceCount()).rejects.toThrow(
      /Timed out waiting for OnboardResponse/
    );
  });
  ...
  test('runtime snapshot reports nonce pool state once peers are hydrated', async ({
    ...
    expect(snapshot.runtime).toBe('ready');
    expect(snapshot.snapshotError).toBeNull();
    assertNoncePoolHydrated('provider-live runtime snapshot', snapshot, 1, 1);
    expect(snapshot.snapshot?.state?.nonce_pool?.peers?.[0]).toMatchObject({
  ...
  test('signEvent succeeds against a live responder after bootstrap hydration', async ({
    ...
    assertNoncePoolHydrated(
      'provider-live pre-sign snapshot',
  ...
  test('signEvent fails cleanly when the live responder disappears mid-session', async ({
    ...
    assertNoncePoolHydrated(
      'provider-live pre-disconnect snapshot',
      await callOffscreenRpc<RuntimeSnapshotResult>('runtime.snapshot'),
      1,
      1
    );
    ...
    await expect(resultPromise).resolves.toMatchObject({
      ...
      message: expect.stringMatching(
        /locked peer response timeout|Failed during onboard request: Onboard response timed out/
      )
    });
  ...
  test('nip44 encrypt fails cleanly when the relay disconnects mid-session', async ({
    ...
    await expect(resultPromise).resolves.toMatchObject({
      ok: false,
      message: expect.stringMatching(
        /locked peer response timeout|Failed during onboard request: Relay subscription closed before onboard response/
      )
    });
