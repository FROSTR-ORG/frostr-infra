...
case "${ACTION}" in
    background)
        ...
        if command -v xcrun >/dev/null 2>&1; then
            xcrun simctl ui booted scroll down 2>/dev/null || true
            # Use simctl to press home button
            xcrun simctl spawn booted launchctl distmate background 2>/dev/null || \
            xcrun simctl spawn booted /usr/bin/isfsld 2>/dev/null || true
        fi
    ...
    foreground)
        ...
        if command -v adb >/dev/null 2>&1; then
            adb shell am start -W -n com.frostr.igloo.dev/com.frostr.igloo.MainActivity 2>/dev/null || true
        fi
        ;;
    *)
